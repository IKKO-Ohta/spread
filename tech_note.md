# 対戦管理・分析アプリケーション spread をセルフホスティングしよう
 ...
 - セルフホスティングする人へ： 利用者の情報を守るため、必ず Firebase セキュリティルール の節を読むようにしてください。
...
spreadは"サーバレス"なアプリケーションですが、本当にサーバ上の処理を何にも行なっていないのかといえばそうではありません。実際、投稿されたデータの保存と取得、認証、招待メールの送信といった操作はサーバで行うのが自然です。そこで、Google Firebaseを利用し、最低限必要な機能をまかなっています。
 ...
 - Firebaseを用いる副次的なメリットとして、バックエンド側のコードを一つのリポジトリにまとめられるという良さもあります。すなわち、あたかもIDLで定義したかのように、フロント側で作成したTypeScriptの型定義を一部の機能で使いまわせるということです。
 - DeployまでFirebase Hostingに面倒を見てもらっています。この方法ではサーバコードやセキュリティルールをも一括でデプロイできます。サーバコードを含むこのプログラムでは、一括のデプロイは欠かせない要件だと考えています。
...
利用者の情報を守るため、セルフホスティングを行う人はこの項目を必ず読んでください。
...
データベースへのアクセスは様々なルールによって制約されますが、もっとも代表的な制約は`ユーザが認証されているかどうか` です。
...
これは[もっとも典型的な制約ルール](https://firebase.google.com/docs/rules/basics?hl=ja#all_authenticated_users)です。これを見るとデータベースの階層ごとに、読み書きを許可できることがわかります。`request.auth.uid`として認証情報を利用できています。
...
```
 - **シート** についてはどんなユーザも読み出すことができます。一方で、新しくシートを作成するためにはログインを必要とします。
 - **ゲーム** についてはどんなユーザも読み出すことができます。一方で、新しくゲームを登録するためにはそのシートのメンバーになっている必要があります。
