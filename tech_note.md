...
認証にはFirebase Authentificationを用います。Authentificationは多様なログイン手段（Gmail、Facebook、Twitter etc.)をサポートし、ローカライズや見た目などのカスタマイズも簡単なサービスです。導入に必要な手順は、firebaseを設定していれば専用のライブラリを設置するだけでです。

#### Firebase セキュリティ ルール

Firebase Authentificationを利用すると非常にシンプルなログイン機能を実装することができます。しかし、緊密に結びついているFirebase セキュリティ ルールをどのように管理するかはとても重要になっています。このルールは他のmBaaSに比べて一風変わっており、セキュリティ インシデントを起こさないよう注意を払って取り組む必要があります。
